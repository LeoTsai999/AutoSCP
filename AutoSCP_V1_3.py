...
class FileTransferApp:
    ...
    def get_txt_files(self, folder):
        """獲取資料夾中所有 .txt 檔案，排除檔名包含當前日期的檔案"""
        today_date = datetime.now().strftime("%Y%m%d")  # 獲取當前日期，格式為 YYYYMMDD
        return [f for f in os.listdir(folder) 
                if f.endswith('.txt') 
                and os.path.isfile(os.path.join(folder, f)) 
                and today_date not in f]
    ...
    def transfer_files(self, folder_a, folder_b, folder_c, password):
        ...
        try:
            ...
            if not txt_files:
                self.update_log_display("資料夾 A 中沒有符合條件的 .txt 檔案")
                logging.info("資料夾 A 中沒有符合條件的 .txt 檔案")
                return
